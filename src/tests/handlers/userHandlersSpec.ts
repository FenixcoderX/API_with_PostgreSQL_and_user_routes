...
describe('Test endpoint /users: ', () => {
  ...
  it('/users , method GET (show all users)', async () => {
    const response = await request.get('/users').set('Authorization', `token ${token}`);
    expect(response.status).toBe(200);
    expect(response.body.length).toEqual(1);
  });

  it('/users/:id , method GET (show a specific user)', async () => {
    const response = await request.get('/users/1').set('Authorization', `token ${token}`);
    expect(response.status).toBe(200);
    expect(response.body.username).toEqual('username');
  });

  it('/users/:id , method PUT (update a user)', async () => {
    const response = await request.put('/users/1').set('Authorization', `token ${token}`).send({
      username: 'username',
      firstName: 'firstName',
